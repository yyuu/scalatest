...
private[scalatest] sealed abstract class SuperEngine[T](concurrentBundleModResourceName: String, simpleClassName: String)  {
        ...
        if (!stopRequested()) {
          node match {
            case testLeaf @ TestLeaf(_, testName, testText, _, _, _, _) =>
              val (filterTest, ignoreTest) = filter(testName, theSuite.testTags, theSuite.suiteId, theSuite.suiteTags)
              if (!filterTest)
                if (ignoreTest) {
  ...
  def runTestsImpl(
    ...
    testName match {
      case Some(tn) =>
        val (filterTest, ignoreTest) = filter(tn, theSuite.testTags, theSuite.suiteId, theSuite.suiteTags)
        if (!filterTest) {
          if (ignoreTest) {
