...
trait StateSuite extends Suite {
  ...
  private def reportTestStarting(theSuite: Suite, report: Reporter, tracker: Tracker, testName: String, testText: String, rerunnable: Option[Rerunner], location:Option[Location]) {
 ...
 private def reportTestFailed(theSuite: Suite, report: Reporter, throwable: Throwable, testName: String, testText: String,
  ...
  private def reportTestSucceeded(theSuite: Suite, report: Reporter, tracker: Tracker, testName: String, testText: String, duration: Long, formatter: Formatter, rerunnable: Option[Rerunner], location:Option[Location]) {
  ...
  private def reportTestIgnored(report: Reporter, tracker: Tracker, testName: String, testText: String, level: Int, location:Option[Location]) {
    val testSucceededIcon = "-"
  ...
  private def handleFailedTest(throwable: Throwable, testName: String,
    ...
    report(TestFailed(tracker.nextOrdinal(), message, suiteName, suiteID, Some(getClass.getName), testName, testName, Some(throwable), Some(duration), Some(formatter), Some(SeeStackDepthException), None))
  }
  override def runTest(testName: String, reporter: Reporter, stopper: Stopper, configMap: Map[String, Any], tracker: Tracker) {
    ...
    if (!testStatuses(simpleName)(testName).isInstanceOf[Ignored])
      reportTestStarting(this, reporter, tracker, testName, testName, None, getTopOfMethod(testName))
    ...
    testStatuses(simpleName)(testName) match {
      case Pending(duration, remaining) =>
        ...
        reportTestPending(this, reporter, tracker, testName, testName, duration, formatter, getTopOfMethod(testName))
      case Ignored(duration, remaining) =>
        if (remaining > 1)
          testStatuses(simpleName)(testName) = Ignored(duration, remaining - 1)
        else
          testStatuses(simpleName)(testName) = Succeeded(duration)
        reportTestIgnored(reporter, tracker, testName, testName, 1, getTopOfMethod(testName))
      case Canceled(duration, remaining) =>
        if (remaining > 1)
          ...
          val formatter = getIndentedText(testName, 1, true)
          reporter(TestCanceled(tracker.nextOrdinal(), message, suiteName, suiteID, Some(getClass.getName), testName, testName, Some(e), Some(duration), Some(formatter), Some(getTopOfMethod(testName)), None))
      ...
      case Succeeded(duration) => reportTestSucceeded(this, reporter, tracker, testName, testName, duration, formatter, None, Some(getTopOfMethod(testName)))
    }
  }
}
