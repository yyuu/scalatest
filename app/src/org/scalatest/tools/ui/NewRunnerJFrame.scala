 ...
 */
package org.scalatest.tools.ui
 ...
 * @author Josh Cough
 */
private[scalatest] class NewRunnerJFrame(val reportTypesToCollect: ReporterOpts.Set32, scalaTest: ScalaTest) extends
    JFrame(Resources("ScalaTestTitle")) with RunDoneListener with RunnerGUI {

  scalaTest.addReporter(new GraphicRunReporter)    
  ...
  private def initialize() = {
    ...
    runJButton.addActionListener(
      new ActionListener() {
        def actionPerformed(ae: ActionEvent) {
          currentState = currentState.runButtonPressed(NewRunnerJFrame.this)
        }
      }
    )
      ...
      new ListSelectionListener() {
        def valueChanged(e: ListSelectionEvent) {
          ...
          if (rh == null) {
            ...
            currentState = currentState.listSelectionChanged(NewRunnerJFrame.this)
          }
          else {
    ...
    rerunJButton.addActionListener(
      new ActionListener() {
        def actionPerformed(ae: ActionEvent) {
          currentState = currentState.rerunButtonPressed(NewRunnerJFrame.this)
        }
      }
    )
  ...
  private def initializeAboutBox() = {
    ...
    new AboutJDialog(NewRunnerJFrame.this, title2)
  }
  ...
  override def done() {
    usingEventDispatchThread {
      currentState = currentState.runFinished(NewRunnerJFrame.this)
    }
  }
  ...
  def showErrorDialog(title: String, msg: String) {
    ...
    val jd: JDialog = jOptionPane.createDialog(NewRunnerJFrame.this, title)
    jd.show()
  }
  ...
  private class RunnerThread extends Thread {
    override def run() {
      try {
        NewRunnerJFrame.this.scalaTest.run
      }
      finally {
        stopper.reset()
      }
  ...
  private class RerunnerThread(rerunnable: Rerunnable) extends Thread {
    ...
    override def run() {
      try{
        NewRunnerJFrame.this.scalaTest.rerun(rerunnable)
      }finally{
        stopper.reset()
        NewRunnerJFrame.this.done()
      }
    }
  }
...
private[scalatest] object NewRunnerJFrame {  
  private val RUNNER_JFRAME_START_X: Int = 150
  private val RUNNER_JFRAME_START_Y: Int = 100
  
  def run(scalaTest: ScalaTest){
      
    val abq = new ArrayBlockingQueue[NewRunnerJFrame](1)
    usingEventDispatchThread {
      val rjf = new NewRunnerJFrame(ReporterOpts.allOptions, scalaTest) 
