...
// they are next to ST commands. So I say  "   <pre>" sometimes instead of " * <pre>".
  ...
  def genTableForNs(outputDir: File) = {
    val tableFor1 = new File(outputDir, "TableFor1.scala")
    val bw = new BufferedWriter(new FileWriter(tableFor1))
  ...
  def genPropertyChecks(outputDir:File) = {
    val propertyChecksFile = new File(outputDir, "TableDrivenPropertyChecks.scala")
    val bw = new BufferedWriter(new FileWriter(propertyChecksFile))
  ...
  def genTables(outputDir:File) = {

    val tablesFile = new File(outputDir, "Tables.scala")
    val bw = new BufferedWriter(new FileWriter(tablesFile))
  ...
  def genTableSuite(outputDir:File) = {

    val tableSuite = new File(outputDir, "TableSuite.scala")
    val bw = new BufferedWriter(new FileWriter(tableSuite))
  ...
  def genCompile(dir:File): List[File] = {
    val outputDir = new File(dir, "scala/org/scalatest/prop")
    outputDir.mkdirs()
    List(genTableForNs(outputDir), genPropertyChecks(outputDir), genTables(outputDir))
  }

  def genTest(dir:File): List[File] = { 
    println("dir: " + dir)
    val outputDir = new File(dir, "scala/org/scalatest/prop")
    outputDir.mkdirs()
    List(genTableSuite(outputDir))
  }
