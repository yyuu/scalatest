...
// they are next to ST commands. So I say  "   <pre>" sometimes instead of " * <pre>".
  ...
  def genPropertyChecks(srcDir: File) = {

    val f = new File(srcDir, "GeneratorDrivenPropertyChecks.scala")
  ...
  def genGeneratorDrivenSuite(srcDir: File, 
   mixinInvitationStyle: Boolean, withTables: Boolean, doItForCheckers: Boolean) = {
    ...
    val f = new File(srcDir, suiteClassName + ".scala")
  ...
  def genCompile(dir:File) = {
    val srcDir = new File(dir, "scala/org/scalatest/prop")
    srcDir.mkdirs()
    List(genPropertyChecks(srcDir))
  }

  def getTest(dir:File) = {
    val srcDir = new File(dir, "scala/org/scalatest/prop")
    srcDir.mkdirs()
    List(
      genGeneratorDrivenSuite(srcDir, mixinInvitationStyle=true,  withTables=false, doItForCheckers=false),
      genGeneratorDrivenSuite(srcDir, mixinInvitationStyle=false, withTables=false, doItForCheckers=false),
      genGeneratorDrivenSuite(srcDir, mixinInvitationStyle=true,  withTables=true,  doItForCheckers=false),
      genGeneratorDrivenSuite(srcDir, mixinInvitationStyle=false, withTables=true,  doItForCheckers=false),
      genGeneratorDrivenSuite(srcDir, mixinInvitationStyle=true,  withTables=true,  doItForCheckers=true),
      genGeneratorDrivenSuite(srcDir, mixinInvitationStyle=false, withTables=true,  doItForCheckers=true)
    )
  }
}
